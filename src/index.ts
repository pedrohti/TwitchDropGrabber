...
async function findRandomChannel(page: Page) {
  ...
  info(chalk.green(`Channel found: navigating to ${chalk.bold(channel)}`));
...
async function isLive(channelPage: Page) {
  ...
  const videoDuration = await channelPage.$$eval('video', (videos) => (videos.pop() as HTMLVideoElement)?.currentTime);
...
async function checkLiveStatus(channelPage: Page) {
  const { videoDuration, notLive, raid, streamingGame } = await isLive(channelPage);
  if (notLive || raid) {
